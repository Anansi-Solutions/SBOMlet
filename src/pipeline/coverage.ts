...
/**
 * Terraform skip arm. The init-has-run gate is a FILESYSTEM signal: modules.json
 * presence-as-a-regular-file + the `.terraform/providers/`+`.terraform/modules/` artifact shape
 * decide, with no HCL parsing. It mirrors the collector EXACTLY via the shared {@link
 ...
 *   scan; if the lock yields zero providers too, skip with a reason (a providers-empty dir is a
 *   legitimate no-op, not a loud failure).
 * - PRESENT (regular file): the size gate fires BEFORE the read. Then terraformComponentCount === 0
 *   (zero-provider lock with a local-only modules.json) → skip with a reason; a positive count →
 *   undefined: scan.
 */
...
/**
 * Skip classification of the coverage policy, pure and shared: the one place that decides a
 * lockfile has nothing to inventory. Both the loop's pre-scan warn+skip branch and classifyCoverage
 * delegate here, so the two checks can never drift apart.
 *
 ...
 *   scan. (Routing the never-init'd case to a skip-to-zero would be the exact
 *   silent-incomplete-inventory failure this tool prevents.)
 *
 * Returns the human-readable reason for the warning line, or undefined when the target must be
 * scanned. lockfileDir is required for the terraform arm (its init-has-run gate is a filesystem
 * fact, not lockfile text); the text-only kinds ignore it.
 */
