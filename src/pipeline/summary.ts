/**
 * The stderr policy summary and the log sanitizer it depends on: every untrusted string printed to
 * stderr passes through sanitizeForLog here, so the fixed summary line shapes cannot be forged or
 * erased.
 */
...
/**
 * Maximum printed length of one untrusted field on stderr: long enough for any legitimate
 * reason/purl, short enough that a crafted megabyte policy reason cannot flood CI logs.
 */
 ...
 * field is length-capped. Plain printable text passes through unchanged, so the fixed summary
 * shapes below are unaffected. Exported for direct unit testing.
 */
 ...
 * reason) passes through sanitizeForLog so the line structure of this summary cannot be forged or
 * erased.
 */
export function writePolicySummary(
  ...
  // Surface the imprecise count on its OWN line (the counts-line shape above is unchanged).
  // Imprecise verdicts are a subset of the warn count - they need clarify-disambiguation, not a
  // gate failure. Printed only when any exist, so policies with no imprecise findings keep
  // byte-identical output.
  //
  // This count is PER-OCCURRENCE (one per package×target verdict), matching the `(M verdicts)`
