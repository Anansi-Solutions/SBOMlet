/**
 * The dedicated `generate-docker-sbom` orchestrator: the ONLY path in the tool that touches the
 * docker daemon / syft / buildx. It resolves ONE of three mutually exclusive lanes, builds when the
 * lane calls for it, scans each resulting image with the single-posture collector
 * (collectDockerOsSbom), and writes the deterministic committed `docker.sbom.json` at the base-dir-
 ...
 *
 * The single writeFileSync lives here (mirroring runGenerate's writer-owns-bytes posture) so the
 * collector stays write-free and a double-run from the same inputs is byte-identical by
 * construction (toSortedJson via emitDockerOsDoc).
 */
...
/**
 * Defense-in-depth ref guard: an image operand forwarded to syft must never be
 * empty/whitespace-only (a no-op operand) nor dash-prefixed (a token syft could parse as a flag).
 ...
 * `[docker]`-ignored ones. Identities and repoRoot are routed through sanitizeForLog before
 * reaching stderr.
 */
...
/**
 * PURE --list-dockerfiles resolution (NO docker, NO syft, NO writes): walk `repoRoot` and return
...
/**
 * PURE targeted-lane resolution (NO docker, NO syft, NO file reads): take the EXPLICITLY named
...
export interface GenerateDockerSbomOptions {
  /**
   * IMAGE LANE (--image): scan pre-existing image refs with the live syft+docker path (pull when a
   * ref is absent locally). One of the three ways in; mutually exclusive with
   * repoRoot/dockerfilePaths (validated in the CLI).
   */
  images?: string[];
  /**
  ...
  repoRoot?: string;
  /**
   * TARGETED BUILD LANE (--dockerfile): build each EXPLICITLY named Dockerfile (base-dir-resolved)
   * to its deterministic tag and scan the built image. The file-list counterpart to `repoRoot`
   * discovery; a missing named path fails fast. Mutually exclusive with the other lanes.
   */
  ...
  policyPath?: string;
  /**
   * Optional override for the committed OS-SBOM output path. When unset it defaults to
   * DOCKER_SBOM_FILE inside the resolved cache dir (the policy `[cache] dir`, or
   * DEFAULT_CACHE_DIR), the same file generate and check read.
   */
  ...
  baseDir?: string;
  /**
   * This tool's OWN directory, excluded from the discovery walk (discovery lane only). Wired
   * exactly as the lockfile discovery path does (targets.ts computes
  ...
  verbose?: boolean;
  /**
   * --list-dockerfiles: print the policy-aware discovered Dockerfile identities to stdout, one per
   * line, and return before any build or write. This is the CI workflow's discovery surface, so the
   * build set comes from the tool's own walk, never a hand-rolled shell find. Requires repoRoot
   * (validated in the CLI).
   */
 ...
 * build context resolve against `cwd`. The DISCOVERY lane anchors it to the repo root (its
 * identities are repo-relative); the TARGETED lane leaves it undefined so a caller's `--dockerfile`
 * path resolves against their own process cwd. `exec` is injectable so the lane wiring is testable
 * without a docker daemon; it defaults to the real execTool seam.
 */
...
/**
 * TARGETED BUILD LANE (--dockerfile): resolve the named Dockerfiles (sorted, deduped, fail-fast on
 * a missing path), build each to its deterministic tag, and scan the built images. The summary
 * prints to stderr BEFORE the build so the resolved set is visible even if a later build/scan
 * fails.
 */
...
/**
 * DISCOVERY BUILD LANE (--repo-root): walk the repo for Dockerfiles, build each from its DISCOVERY
 * IDENTITY STRING (the committed-identity input), and scan the built images. A build set emptied by
 * `[docker] ignore` / --exclude is a loud error, matching the old script's exit-1 posture.
 * repoRootOpt is threaded as its own parameter since the dispatch guard narrows opts.repoRoot only
 * at the call site.
 */
async function runDiscoveryBuildLane(
  ...
  // to prepare context: path not found".
  await buildImages(
    build.map((b) => b.identity),
    opts.verbose ?? false,
    repoRoot,
 ...
 * the whole set being unsafe is a loud throw, never a silent empty scan. The collector probes local
...
export async function runGenerateDockerSbom(
  ...
  if (opts.listDockerfiles === true) {
    ...
    // the same invariant at this public API boundary so a programmatic caller can never fall
    // through into a build/scan lane.
  ...
  // Default output: DOCKER_SBOM_FILE inside the resolved cache dir (the policy `[cache] dir`, or
  // the default, anchored to the scanned repo), so the file this command WRITES is exactly the one
  // generate and check READ; --docker-sbom overrides it. resolveCacheDir already handles repoRoot:
  // undefined (the --dockerfile / --image lanes have no repo root).
  const outputPath =
