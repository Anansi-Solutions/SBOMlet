/**
 * The check CI gate: the same write-free pipeline as generate, byte-compared against the committed
 * files instead of written. exitCodeFor's violation-beats-stale mapping is the only producer of
 * exit codes 1 and 2.
 */
...
/**
 * check = the same write-free pipeline as generate, compared instead of written: buildOutputs
 ...
 * is SHARED across subcommands, so without this explicit guard check would silently ACCEPT
 * --intensive and gain a scan path the offline gate must never have.
 */
export async function runCheck(opts: GenerateOptions): Promise<CheckResult> {
  ...
  // The comparison set reads the same base-dir-resolved paths generate writes: a relative CYCLONEDX
  // var must never make check "verify" a file at a different location than the one the user
  // committed.
  ...
  // A miss-needing-enrichment with no committed cache entry is stale by the same logic as a missing
  // output: check cannot fetch it, so the committed cache is out of date. Name the purl and the
  // regenerate remedy. These join staleFiles so exitCodeFor maps them to exit 2.
