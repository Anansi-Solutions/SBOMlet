/**
 * syft Docker OS-package collector - a HYBRID of the cdxgen adapter (exec + version pin +
 * specVersion assert) and the terraform collector (minimal deterministic CycloneDX re-emit + size
 * gate). It is GENERATE-ONLY and is NOT a registry Collector: `gate/check.ts` runs collectTargets
 * on the check path, so a registered collector would force a docker daemon onto every CI check. Its
 * committed SBOM is consumed as a merge INPUT by the pipeline, never discovered per-run.
 *
 * Why syft: a comparison spike measured syft filling 96.7% dpkg + 100% apk licenses where cdxgen
 * `-t docker` fills 0% dpkg - and OS-package + license extraction across BOTH dpkg and apk is the
 ...
 * no timestamp). Each component carries `images` - the sorted membership of `dockerImages[].image`
 * values it was seen in - and each dockerImages entry carries `source`, the identity the image came
 * from (the Dockerfile identity for built images, the requested ref verbatim otherwise). Serialized
 * via the tool-wide `toSortedJson` contract so a double-emit is byte-identical by
...
/**
 * Collector tool identity. The literal version is the pin - it is mirrored in mise.toml
 * (`aqua:anchore/syft@1.45.1`) and asserted by test/dockerOs.test.ts so a version bump must be
 * conscious. Re-verify against `mise x -- syft version` when refreshing (07-RESEARCH "valid until
 * ~2026-07-15").
 */
 ...
 * 64 MiB is generous headroom. The stat gate fires before any read/parse so a hostile/huge SBOM can
...
/**
 * Stat-gate a syft SBOM path against MAX_SYFT_SBOM_BYTES BEFORE any read or parse. Mirrors
 * terraform.ts assertTerraformLockSize.
 */
...
/**
 * The verified syft 1.45.1 argv. Options come FIRST (the `cyclonedx-json=<file>` output target so
 * syft writes deterministic JSON to the per-run temp file), then a `--` END-OF-OPTIONS separator,
 * then the image OPERAND last. The image ref is always an argv operand (never a shell string
 * - command injection is impossible by construction); the `--` is DEFENSE-IN-DEPTH so that even a
 * defensively dash-prefixed ref can never be parsed by syft as a flag. syft accepts
 * `syft -o <fmt>=<file> -- <image>` (verified against syft 1.45.1). The literal `cyclonedx-json`
 * format is grep-detectable and locked byte-for-byte by test/dockerOs.test.ts - any flag change
 * must consciously break that test and invalidate the committed OS-SBOM goldens.
 */
...
/**
 * docker inspect argv that prints the image's RepoDigests JSON array to stdout. The image is the
 * OPERAND after a `--` END-OF-OPTIONS separator (symmetry with syftArgs): a dash-prefixed operand
 * can never be parsed by `docker inspect` as a flag. The ref is always an argv operand, never a
 * shell string - command injection is impossible by construction.
 */
...
/**
 * docker pull argv for the probe-first implicit pull. The image is the OPERAND after a `--`
 * END-OF-OPTIONS separator (symmetry with dockerInspectArgs/ syftArgs): a dash-prefixed operand can
 * never be parsed by `docker pull` as a flag, and the ref is always an argv operand, never a shell
 * string. Pull runs ONLY when the presence probe finds the image absent locally - a present tag is
 * scanned as-is, so the network is never raced into the determinism contract and a stale local tag
...
/**
 * The three CycloneDX license-claim shapes syft emits per OS package. These are EXACTLY the shapes
 * the merge's `licenseClaimsOf` reads (validate/sbom.ts: SbomExpressionClaim / SbomIdClaim /
 * SbomNameClaim), so preserving them verbatim is what makes OS packages render REAL licenses
 * instead of unknown.
 */
...
/**
 * One retained OS component: the minimal deterministic emit shape. The `licenses` array is the
 * syft-resolved CycloneDX license claims, PRESERVED so the merge picks them up (the whole reason
 * syft was chosen - 96.7% dpkg / 100% apk fill). Omitted entirely when syft resolved no license for
 * the package (field-absent, never an empty array, so the field stays meaningful and the emit
 * byte-stable).
 */
...
/**
 * One EMITTED component: an OsComponent plus its cross-image membership. `images` is the sorted
 * list of `dockerImages[].image` values the component was seen in. Attribution attaches ONLY in the
 * cross-image union ({@link unionOsComponents}) - {@link filterOsComponents} stays per-image and
 * membership-free - so the emitted doc always says which image(s) each component came from.
 */
...
/**
 * The image→platform-digest sidecar entry. `source` records the identity the image came from: the
 * Dockerfile identity for built images (the exact string `--list-dockerfiles` prints), the
 * requested ref verbatim for pre-existing images (source === image). A digest never appears in a
 * source - digests live in `digest`, so a re-pin can never churn an identity.
 */
...
/**
 * One image to scan: the ref handed to syft/docker as an operand plus the source identity it stands
 * for, carried into the emitted `dockerImages[].source` (see {@link DockerImageDigest}).
 */
...
/**
 * Narrow one raw `licenses[]` entry to a known CycloneDX claim shape, returning the normalized
 * OsLicense or undefined for any foreign/malformed entry (which is then dropped). The
 * id/name/expression order mirrors licenseClaimsOf's precedence: an entry carrying BOTH license.id
...
/**
 * Canonical sort key for a normalized OsLicense: a discriminator (`0:` id, `1:` name, `2:`
 * expression) plus its value. syft's per-component license order is not guaranteed stable, so
 * sorting by this key makes the re-emit byte-identical regardless of syft's emission order (the
...
/**
 * Extract + normalize + deterministically sort a component's syft license claims. Returns undefined
 * (field-absent) when the component carries no recognizable license - never an empty array - so the
 * emit shape stays meaningful and byte-stable.
 */
...
/**
 * True iff the raw component carries a non-empty string name+version+purl, across ALL ecosystems
 * (deb/apk/npm/pypi/...) - the single full-image-contents predicate. Drops syft's purl-less
 * file/operating-system/generic noise and any component with an empty name/version/purl.
 */
...
/**
 * Filter syft's raw components down to the retained set, purl-sorted and purl-deduped first-wins
 * (mirrors terraform.ts componentsOf) - the whole emission is a pure function of bytes.
 *
 * ONE posture: keep EVERY component carrying a non-empty string name+version+purl, across all
 * ecosystems (deb/apk/npm/pypi/...) - the full image contents. syft's purl-less
 * file/operating-system/generic noise and empty-name/version/purl entries are dropped.
 */
...
/**
 * Cross-image membership UNION: fold each image's filtered components into one purl-keyed set where
 * a purl shared between images is ONE row whose `images` lists every containing image (sorted
 * before return). The retained name/version/licenses are the FIRST-SEEN values - callers pass
 * images in compareCodeUnits-sorted order (collectDockerOsSbom's scan discipline), so the license
 * posture stays first-wins-by-sorted-image: unchanged from the pre-membership dedup, just visible
 * now that the row names both images.
 */
  ...
  for (const { image, components } of perImage) {
    for (const component of components) {
      ...
      if (existing === undefined) {
        ...
        // Purl hit: union the membership, keep the first-seen fields. The includes guard keeps a
        // duplicated ref in the scan set from double-counting one image.
 ...
 * `{ bomFormat, specVersion:"1.6", components, dockerImages }` and nothing else - no serialNumber,
 * no metadata, no timestamp. Components carry their sorted `images` membership; dockerImages
 * entries carry `source` alongside image/digest, and the sidecar is sorted by image. Uses the
 * tool-wide `toSortedJson` contract (sorted keys, LF, indent 2) so a double-emit from the same
...
/**
 * Scan one image with syft and return its raw SBOM path. The size gate fires before any read;
 * specVersion is asserted with an actionable error naming the full invocation (cdxgen.ts
 * pin-verification idiom).
 */
 ...
 * (repo-match, else compareCodeUnits-smallest) so the committed docker.sbom.json is byte-stable
 * across machines - NOT the daemon-order-dependent `digests[0]`, and NOT the manifest-list digest a
 * `buildx imagetools inspect` would return. Returns "" when the image has no RepoDigests (a
 * local-only / never-pushed build - e.g. a just-built tag), which is the generalized digest-less
 * identity.
 */
 ...
 * `docker inspect --format '{{json .RepoDigests}}'` returns a daemon-ORDER-dependent array: an
 * image pulled from / pushed to multiple registries carries multiple RepoDigests whose array order
...
/**
 * The repository portion of an image ref or a RepoDigest - everything before an `@sha256:` digest
 * and before any `:tag`. Returns undefined for an empty input. Used to match a requested image ref
 * against a RepoDigest's repository so the selected digest is the registry the user asked about. A
 * port in a registry host (`registry:5000/app`) is preserved: only a `:tag` AFTER the last `/` is
 * stripped.
 */
function repositoryOf(ref: string): string | undefined {
  ...
  // Strip a trailing `:tag` only when the colon is AFTER the last `/` (so a registry host:port
...
/**
 * Orchestrate per-image scan + digest + filter, union all images' components into one purl-keyed
 * sorted list carrying per-image membership, build the sorted dockerImages sidecar (each entry
 * carrying its source identity), and
 * return the deterministic doc plus the per-image SBOM temp paths. Async for
 * interface symmetry with the other collectors.
 *
 * Images are scanned in compareCodeUnits-SORTED order (never the caller's argv order): the
 * cross-image union keeps the FIRST-SEEN license claims, so when two images share a purl with
 * DIFFERENT claims, whichever image is scanned first decides the committed license - an unsorted
 * walk makes that choice a function of argv order, breaking the byte-determinism contract for
 * `--image b a` vs `--image a b` over the identical image SET. Sorting here matches the existing
 * resolveDiscoveredImages / resolveTargetedDockerfiles convention (both already
 * compareCodeUnits-sort before scanning); this closes the one caller path (--image) that did not.
 */
/**
 * Scan + filter + identify ONE image with the single posture: probe local presence, implicitly pull
 * when absent, scan the FULL contents, then resolve the digest (pinned when RepoDigests exist, ""
 * when they do not). Extracted from collectDockerOsSbom to keep that orchestrator under the
 * complexity bound (mirrors the resolveLiveScanImages / run*Mode extraction precedent).
 */
async function collectOneImage(
  ...
  // Probe-first implicit pull: a locally-present ref is scanned as-is (never re-pulled, so a stale
  // tag is never silently refreshed); only an absent ref is pulled, and if the pull fails the error
  // surfaces BEFORE any scan or digest resolution ("" can never mask a typo'd ref).
...
/**
 * True iff `docker inspect <image>` exits zero - the image is present in the local daemon. A
 * nonzero exit (execTool rejects) means absent; we never parse `docker images` output (a fragile,
 * format-unstable hand-roll). The same `docker inspect` the digest step runs, used here only for
 * its exit code.
 */
