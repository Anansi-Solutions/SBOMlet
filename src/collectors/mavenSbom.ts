 ...
 *
 * The design that survives is the docker.sbom.json pattern generalized into a lockfile kind: the
 * consumer's own CI runs the pinned, ecosystem-standard cyclonedx-maven-plugin and commits its
 ...
 *
 * First-party exclusion for multi-module reactors (a sibling module appearing as a plain component
 ...
 * `label` (the file's own name) appears in every error message so a failure always names which of
 * the two sidecars misparsed.
 */
...
/**
 * Read a target's maven.sbom.json (no subprocess, no cwd change) and emit the inventory the merge
 * consumes. When no maven.test.sbom.json sidecar sits beside it, the committed bytes are copied
 * VERBATIM (never re-serialized). When a test doc IS present, the two are composed (see {@link
 * composeMavenInventory}) and `prodPurlSet` is derived from the default doc's own purls, both
 ...
 *   expectation;
 * - a document whose metadata.component.purl is not `pkg:maven/...` → loud error naming both the
 *   found purl and the expected prefix (a deliberate
 *   wrong-ecosystem file committed under this name);
 * - a test doc whose root purl differs from the default doc's → loud error naming both roots (a
 *   stale or wrong-module pair must never compose).
 *
 * maven.test.sbom.json is entirely optional-additive: its absence changes nothing about the default
 * doc's failure ladder or output bytes.
 */
export async function collectWithMavenSbom(
  ...
  if (hasTestDoc) {
    ...
    // The pair must be two builds of the SAME module. The merge excludes the inventory component
    // matching a document's own root purl, so a test doc whose root names a real dependency (a
    // stale, wrong-module, or crafted file) would silently drop that dependency from the inventory.
