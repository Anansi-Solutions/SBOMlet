 ...
 * (asking the registration for its tool identity first), so the fixed stderr shapes are emitted
 * from exactly one place; ctx.log is the loop-provided sink should a collector ever need to surface
 * a line through the CLI.
 */
...
export interface Collector {
  /**
   * Tool identity for the stderr line, resolved from the lockfile text: yarn's generator choice is
   * content-dependent (plugin for Yarn 4+, cdxgen otherwise); every other kind ignores the
   * argument.
   */
...
/**
 * cdxgen-family collector: ecosystem and cache-key manifest list come from the dispatch tables for
 * the registered kind; the optional firstParty derivation supplies the lockfile-derived member-name
 * set the merge pairs with its second first-party signal.
 */
...
const yarnCollector: Collector = {
  ...
  async collect(target, ctx): Promise<CollectedSbom> {
    ...
    // Read the full SBOM once and derive provenance from its complete root-anchored dependency
...
/**
 * bun targets use the in-process bun.lock collector (no upstream generator preserves bun.lock
 * identity). No subprocess, so timeoutMs is ignored; the lockfile size gate fires inside
 * collectWithBunLock as well as in the CLI loop. NO firstPartyNames: the collector already excluded
 * @workspace: members and workspaces[*].name twins itself.
 */
...
/**
 * poetry targets run cdxgen for the component inventory but derive dev/prod scope from poetry.lock,
...
/**
 * terraform targets use the in-process Terraform collector (no upstream generator resolves
 * Terraform provider/module licenses). No subprocess, so timeoutMs is ignored. requireLockfileText
 * asserts the loop passed the lock text; the collector reads .terraform/modules/modules.json from
 * target.dir itself (not from ctx) and FAILS LOUD when it is absent (the init-has-run gate). NO
 * firstPartyNames / NO prodPurlSet: the collector excludes local modules itself and emits a flat
 ...
 * so name-collision exclusion is meaningless here; reactor sibling exclusion is a separate
 * cross-target pre-pass, not this registration's job. prodPurlSet IS threaded through when the
 * collector derives one: a module committing both maven.sbom.json and maven.test.sbom.json gets its
 * prodPurlSet from the default doc's purls (the yarn dual-run / poetry precedent); a module with
 * only the default doc still carries none, so every component classifies prod exactly as before.
 */
