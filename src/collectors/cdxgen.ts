...
/**
 * Generator target ecosystem: cdxgen scans Python (poetry/uv) targets via `-t python` and JS
 * targets via `-t js`.
 */
...
export interface CollectOptions {
  ...
  verbose: boolean;
  /**
   * Executable that runs the pinned generator. Defaults to "bun" (`bun x` resolves as a real
   * executable on Windows, where npx is npx.cmd and not directly spawnable). Kept as a parameter so
   * the Node fallback path (npx) stays possible.
   */
  runner?: string;
  ...
  ecosystem?: Ecosystem;
  /**
   * Manifest files hashed into the cache key, relative to the target dir. Defaults to the JS pair;
   * poetry targets pass ["poetry.lock", "pyproject.toml"], uv targets ["uv.lock",
   * "pyproject.toml"].
   */
 ...
 * target's content, and any real flag change still flows through this argv and invalidates the key.
 */
 ...
 * manifest-file boundary, name/version cannot blend, and ["a b"] hashes differently from ["a",
 * "b"]. The framing is part of the cache contract and must not change silently. The manifest list
 * is caller-supplied: yarn targets hash ["yarn.lock", "package.json"], poetry targets
 * ["poetry.lock", "pyproject.toml"], uv targets ["uv.lock", "pyproject.toml"]. A yarn WORKSPACE
 * UNIT instead passes explicit {file, dir} entries (root yarn.lock, workspace package.json, root
...
/**
 * The narrow swappable generator interface: a generator is swapped by adding one file satisfying
 * this signature.
 */
