 ...
 *
 * MAIN deps: `[project].dependencies` (PEP 621 array of `"name (constraint)"` strings) is
 * AUTHORITATIVE when present. The legacy `[tool.poetry.dependencies]` table is constraint/source
 * metadata that may list non-top-level names, so it is read as roots ONLY when
...
/**
 * The package name from a PEP 621 dependency spec: the leading run of name-legal characters before
 * the first space, `(`, `[`, or version operator. E.g. "jupyterlab (>=4.5.0,<4.6.0)" →
 * "jupyterlab"; "anyio[trio]>=4" → "anyio".
 */
...
/**
 * Parse poetry.lock into the package list (purl + normalized name + dep table). A package missing a
 * string name or version contributes nothing (cannot form a purl). Garbage input yields an empty
 * list.
 */
...
/**
 * Build the purl-space PurlGraph from the parsed lock packages + declared roots. Every edge
 * resolves the dependency NAME (PEP-503 normalized) to a lock package's purl; a dependency naming a
 * package absent from the lock is dropped (no purl to point at).
 *
...
/**
 * Per-purl provenance for every poetry.lock package. The result is keyed by purl (the merge dedup
 * key); the caller threads each entry onto the matching python occurrence. Empty map for garbage
 * input.
 */
