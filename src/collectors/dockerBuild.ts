 ...
 *
 * imageTag produces the STABLE image identity that becomes the committed sidecar's
 * `dockerImages[].image`: a sanitized, path-hashed tag that is a pure
 * function of the Dockerfile path. It is locked bit-exact by a golden in
 * test/dockerBuild.test.ts against the committed identity, because any drift churns the committed
 * artifact and every consumer's offline check.
 *
 * This module stays silent (no stderr): the caller owns progress logging, the same posture as
 * dockerOs.ts (the pure engine writes nothing; the pipeline / CLI narrate).
 */
 ...
 * Hashing the original (not the sanitized) string is what keeps a mixed-case path distinct from its
 * lowercased twin.
 */
 ...
 * the `-f` value and the build CONTEXT is its POSIX dirname, so the same path produces the same
 * argv on every platform.
 */
...
export interface BuildImageOptions {
   ...
   * is intentionally repo-relative (the `-f` value and the POSIX-dirname build context), so buildx
