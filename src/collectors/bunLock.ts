 ...
 *   parser/cataloger);
 * - trivy parses bun.lock but corrupts identity on nested version-conflict entries, producing purls
 *   that can never resolve as a merge/enrichment key.
 *
...
/**
 * DoS bound: real bun.lock files are <2 MB; 32 MiB is generous headroom. The stat gate fires before
 * any read or parse so a hostile file can never balloon memory.
 */
...
/**
 * Strip trailing commas so machine-written JSONC parses with JSON.parse. The regex only touches a
 * comma directly before a `}`/`]` closer; bun.lock string values (names, semvers, sha512 base64,
 * workspace paths) cannot contain `}`/`]` after a comma, so it is a no-op on strict JSON.
 */
 ...
 * dependencies/optionalDependencies/peerDependencies names as prod roots and its devDependencies
 * names as dev roots.
 *
 ...
 * unrelated top-level "@scope/y".
 *
 * Conservative direction: unknown dep names (no packages entry at any lookup) are silent leaves;
 * unvisited packages stay prod. A visited set bounds the traversal to one visit per packages key;
 * dep edges are read tolerantly from each entry's value[2] metadata object (absent or malformed →
 * leaf).
 */
function transitiveDevKeys(
  ...
  // Roots from every importer. Root names resolve in the importer's own parent context (a one-name
  // chain of its package name) so member-nested conflict keys like "libb/dep" are honored; the
...
/**
 * Emit one component per third-party packages entry (duplicate purls from nested conflict keys
 * kept; the fold to one row happens at merge).
 *
 * First-party exclusion belt-and-braces: workspaces[*].name members are never emitted even if an
 * entry lacks the @workspace: protocol.
 */
...
/**
 * Scan a bun target by reading only bun.lock inside it (no subprocess, no cwd change) and writing a
 * minimal deterministic CycloneDX 1.6 bom.json into the per-run temp dir.
 *
