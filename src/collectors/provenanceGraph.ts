/**
 * Shared purl-space provenance graph + deterministic introducer derivation. Both the npm/yarn lane
 * (BOM `dependencies` graph) and the python lane (poetry.lock `[package.dependencies]` tables)
 * translate their source edges into THIS purl-space adjacency, then call {@link
 * deriveIntroductions} for an identical, deterministic direct/introducedBy/path computation.
 *
 * Determinism is structural: derived purely from the edge SET, never source byte or line order.
 * Every frontier expansion and every emitted set/path is sorted by {@link compareCodeUnits}, so two
 ...
 * single representative `path` is an honest one-of-many for a multi-parent package, never the only
 * introducer.
 */
...
/**
 * Expand one BFS level into the next, marking visited (cycle bound). The result is sorted by purl
 * so the tie-break is stable regardless of parent processing order.
 */
...
/**
 * Deterministic representative shortest root→target purl chain (root-excluded, target-included).
 * BFS seeds the root children sorted by purl; each level is expanded into a frontier re-sorted by
 * CHILD purl ({@link expandLevel}) and a visited set bounds cycles, so the FIRST time `target` is
...
/**
 * The SET of purls REACHABLE from the declared roots over the introducer graph (root children seed
 * the frontier; each node's children expand it). Computed ONCE per derivation. A multi-source BFS
 * bounded by a visited set, so cycles terminate and the result is order-independent (derived from
 ...
 */
/**
 * Expand one reachability frontier into the next, marking newly-discovered purls in `reachable`
 * (the visited set that bounds cycles). Extracted from {@link reachableFromRoots} to keep nesting
 * shallow.
 */
...
/**
 * Per-purl provenance for every node in the graph. Direct nodes carry an empty introducedBy and no
 * path; transitive nodes carry the sorted-unique parent SET (INTERSECTED with the root-reachable
 * set) and a representative tie-broken shortest path (when reachable).
 *
 * CENTRAL REACHABILITY INVARIANT: a node's `introducedBy` may name ONLY parents that are themselves
