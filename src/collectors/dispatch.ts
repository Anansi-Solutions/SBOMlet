 ...
 *
 * npm/pnpm/bun all map to the js ecosystem. npm and pnpm route through the cdxgen adapter; bun
 * targets are collected by the custom bun.lock collector but reuse the same manifest list for
 * cache-key framing.
 *
...
/**
 * Lockfile kind → manifest files hashed into the scan cache key. uv takes the same adapter path and
 * cache-key shape as poetry.
 */
export function manifestFilesFor(kind: LockfileKind): readonly string[] {
  switch (kind) {
    ...
    case "terraform":
      // Only the lock is hashed into the cache key. modules.json is gitignored and absent until
      // init runs (its presence is the loud-fail gate, not a cache input); a re-init that changes
      // resolved module versions also rewrites the lock, so the lock byte-hash is a sufficient
      // cache framing.
      return [".terraform.lock.hcl"];
    case "nuget":
      // Only the lock is hashed into the cache key. *.csproj is a name PATTERN, not a fixed name
      // (computeCacheKey takes fixed names and throws on missing files), and
      // Directory.Packages.props is optional (the pnpm-workspace.yaml precedent). Any project or
      // central-package edit that changes resolution rewrites the lock on the next restore, so the
      // lock byte-hash is a sufficient cache framing.
