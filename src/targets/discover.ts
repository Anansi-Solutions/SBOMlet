 ...
 * workspace list.
 *
 * Identities are forward-slash repo-relative directory paths, copying target.ts's idiom; results
 * sort deterministically by (identity, lockfile kind) using compareCodeUnits, the only comparator
 * allowed tool-wide.
 *
 * Three pure post-steps over the sorted walk output:
 * - Same-dir JS lockfile collisions collapse to one target by precedence bun > pnpm > yarn > npm
 *   with a warning naming the chosen and every ignored lockfile; cross-ecosystem pairs (a JS
 *   lockfile plus poetry/uv in one directory) still yield two targets.
...
/**
 * Same-directory JS lockfile precedence: lower rank wins. A stray package-lock.json is the most
 * common accident (npm auto-writes it), so npm ranks last; bun emits a derived yarn.lock for
 * compatibility, so bun outranks yarn; migration tooling leaves the older-generation file behind,
 * so the newest-generation package manager's lockfile is the deliberate artifact.
 */
 ...
 * is excluded by the leading-"." rule below in {@link shouldDescendDir}. Exported so the reuse is
 * by reference, not by re-hardcoding.
 */
...
/**
 * Dot-directories the DOCKERFILE lane is allowed to descend: the conventional homes of real
 * Dockerfiles. The leading-"." prune is right for the lockfile lane (it keeps
 * .terraform/.yarn/.cache out) but wrongly drops these two; the Dockerfile lane opts into them via
 * {@link shouldDescendDir}'s allowlist param while STILL pruning .git/.terraform/every other
 * dot-dir. Compared case-insensitively for Windows parity.
 */
...
/**
 * The universal walk descent predicate, shared by lockfile and Dockerfile discovery: skip the
 ...
 * - `*` matches within a single path segment (any run of non-`/` chars): "a/*" excludes "a/b" but
 *   NOT "a/b/c".
 * - `**` matches across segments (any run of chars including `/`): "a/**" excludes everything under
 *   "a" but not "a" itself.
 *
 * All other characters are regex-escaped literally; `**` is translated before the remaining single
 * `*` so the two semantics never overlap. Exported so Dockerfile discovery applies byte-identical
 * glob semantics to --exclude and [docker] ignore globs.
 *
 * The regex is CASE-INSENSITIVE (the `i` flag): the platform is Windows (a case-insensitive
 * filesystem) and the literal dir-name exclusion was
...
/**
 * The csproj-sighted-but-no-lock warning strings: a .NET project without a committed
 * packages.lock.json must be TOLD the migration recipe (RestorePackagesWithLockFile=true +
 * `dotnet restore`), never silently inventoried as zero. One aggregated summary line by default;
 * one line per directory when `verbose` (the CLI's --verbose) is set. `lockless` arrives
...
/**
 * The pom.xml-sighted-but-no-sidecar warning strings: a Maven project without a committed
 * maven.sbom.json must be TOLD the adoption recipe (run the pinned cyclonedx-maven-plugin's makeBom
 * goal in CI, commit the result), never silently inventoried as zero. One aggregated summary line
 * by default; one line per directory when `verbose` (the CLI's --verbose) is set. `unsidecared`
 * arrives compareCodeUnits-sorted, so both shapes are deterministic.
 *
 * Identities are repo-author-controlled directory names printed to stderr, so they pass through
 * sanitizeForLog AT RENDER ONLY (a crafted name cannot forge or erase warning lines); the
 * suppression/exclusion matching upstream stays on the raw identities. Exported for direct unit
 ...
 *
 * - One DiscoveredTarget per lockfile found, sorted by (identity, lockfile kind). A directory
 *   holding lockfiles of DIFFERENT ecosystems (e.g. bun.lock and poetry.lock) yields two targets
 *   with the same identity (deterministic via the kind tiebreak).
 * - A directory holding multiple JS lockfiles collapses to exactly one target by precedence bun >
 *   pnpm > yarn > npm, with a warning naming the chosen lockfile and every ignored one.
 * - A bun.lockb sighting with no surviving bun target in its directory yields a warning naming the
 *   migration command; beside a bun.lock target it is silent. Excluded identities produce neither
 *   targets nor bun.lockb warnings.
 */
export function discoverTargetsWithWarnings(
  ...
  // (the text lockfile is authoritative); otherwise warn naming the migration command.
