/**
 * Target resolution: an explicit `--target <path>` argument becomes a validated Target with a
 * deterministic, forward-slash identity string.
 *
 * The identity contract ("libraries/iframe-rpc" shape) is fixed because it surfaces in rendered
 * output.
 */
...
export interface Target {
  ...
  lockfileDir?: string;
  /**
   * Present ONLY on a yarn workspace scan unit: the lock-relative, forward-slash workspace path
   * exactly as `@workspace:` resolved it (e.g. "backend", "libs/a"). Feeds the cache-key
   * discriminator so two workspaces sharing byte-identical manifests never collide.
   */
  workspacePath?: string;
}

/**
 * Walk parent directories looking for a `.git` entry. `existsSync` matches both a `.git` directory
 * and a `.git` file (git worktrees use a file).
 */
