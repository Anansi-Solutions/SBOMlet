...
/**
 * {@link nameOrPatternProblems} in the string-sink shape a `[[clarify]]` entry still validates
 * through.
 */
export function validateNameOrPattern(
  ...
  const result = nameOrPatternProblems(entry);

  for (const problem of result.problems) {
    problems.push(`${where}: ${problem.message}`);
  }

  return result.problems.length === 0 ? { ...result.selector, valid: true } : { valid: false };
}

/**
 * The parsed `version` pin in the string-sink shape a `[[clarify]]` entry still validates through.
 */
...
/**
 * {@link versionPinProblems} in the string-sink shape a `[[clarify]]` entry still validates
 * through.
 */
export function validateVersionPin(
  ...
  const result = versionPinProblems(entry, options);
  ...
  return result.problems.length === 0
    ? { ...(result.version !== undefined ? { version: result.version } : {}), valid: true }
    : { valid: false };
}
