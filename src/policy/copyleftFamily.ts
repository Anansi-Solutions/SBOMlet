 ...
 * this is a SEPARATE, small, enumerated literal token set.
 *
 * MEMBERSHIP: the strong-copyleft GNU-family tokens (GPL/AGPL/LGPL) plus EUPL. A bare "GPL" could
 * be any GPL variant (all copyleft); likewise "AGPL"/"LGPL". "EUPL" is strong copyleft but
 * spdx-correct cross-maps the bare label to a PERMISSIVE id, so it MUST be intercepted here rather
 * than left on the precise path. An imprecise finding with one of these families is
 * flagged-for-review (a warn, never a silent pass) so a maintainer disambiguates it via a
 * `[[clarify]]` override.
 *
 ...
 *   adding speculative tokens would be untested dead data. The precise weak-copyleft ids are
 *   already gated by COPYLEFT_FAMILY on the EXPRESSION path, which an imprecise finding never
 *   reaches.
 *
 * The set is asserted member-by-member in the tests so a typo cannot silently drop a copyleft
 ...
 *
 * A workspace that is ITSELF distributed under a strong-copyleft license can absorb the obligations
 * of inbound copyleft dependencies it bundles, because the whole workspace is re-released under
 * that strong copyleft. apps/scratch is re-released under AGPL-3.0, so it absorbs not only the
 * exact GNU family (GPL/LGPL/AGPL) but ALSO the weaker/compatible copyleft families actually
 * bundled there (notably MPL via the sharp-libvips Apache/LGPL prebuilds and the MPL-licensed
 ...
 * CC-BY-SA (content ShareAlike) are NOT inbound-compatible with an AGPL software re-release and are
...
> = new Map([
  [
    ...
    new Set([
      ...
      "GNU",
      // MPL: file-level weak copyleft; an MPL file relicensed/bundled into an AGPL work is
      // compatible with (and absorbed by) the stronger AGPL obligation. Present in apps/scratch via
      // Apache/LGPL+MPL transitives.
      "MPL",
    ]),
  ],
]);
