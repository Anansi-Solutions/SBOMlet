 ...
 * repository consuming this tool denies them without authoring a single
 * [[deny]] entry, mirroring the shipped copyleft families (copyleft.ts) and
 * the shipped clarify defaults (builtinOverrides.ts). A compliance tool whose
 * out-of-the-box behaviour passes a source-available license has its defaults
 * backwards — this is the corrective.
 *
 * A consumer's own [[deny]] entries are evaluated alongside these: the deny
 * election (see docs/glossary.md#election) spans both sets (denylist.ts), so
 * a policy deny and a builtin deny compose correctly. Attribution is
 * policy-first — a license a consumer also lists is cited as their
 * `denied[i]`; a license only these defaults catch is cited as
 ...
 * stay name-mode opt-ins in the consumer's policy: a name-mode default would
 * have to guess encumbered package names, which the deny matcher must never do.
 */
