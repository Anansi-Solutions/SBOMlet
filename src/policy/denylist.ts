 ...
 * above stale, compatible, workspace suppression, and the dev-scope downgrade.
 * A use-restricted ("source-available") license (BUSL/SSPL/Elastic/RSAL) or a
 * use-restriction rider (Commons-Clause) legally cannot be redistributed in
 * client-shipped artifacts, so no other policy lever can license it back in.
 * Deny mirrors compatible: compatible elects a package in, deny elects it out
 * (see docs/glossary.md#election for what "elect" means here).
 *
 * The effective deny set has two sources but one precedence lane: the
 * consumer's policy `[[deny]]` entries, plus the shipped source-available
 * defaults (builtinDenylist.ts). A source-available license fails out of the
 * box with no policy authored, exactly like the shipped copyleft families and
 * clarify defaults. effectiveDenyRules orders policy entries first, so a
 * license a consumer also lists wins attribution to their explicit
 * `denied[i]`; a license only the defaults catch is cited
 * `default:source-available`. The deny election spans both sets, so the two
 * compose correctly.
 *
 ...
 *     Never substring, never re-parsed at evaluate time. RSAL has no
 *     registered SPDX id, so it ships in name-mode, not here.
 *
 *   match = "name": `pattern` is a verbatim, case-sensitive package-name
 *     compare. This is the escape hatch for non-SPDX use-restriction riders
 *     like Commons-Clause, which is not a registered SPDX license and rides
 *     alongside another license (e.g. "MIT AND Commons-Clause" — not
 *     SPDX-parseable). The spdx-satisfies path cannot catch it; an exact name
 *     compare can. Name-mode deliberately does not require a parseable
 *     license expression — a package with an unknown (null) finding can still
 *     be name-denied. The compare is exact, never a broad regex or substring,
 *     so a typo'd or unrelated name can never be denied. The shipped defaults
 *     are license-mode only — a name-mode default would have to guess
 *     encumbered package names.
 *
 * Election applies to deny too, not just to compatible (load-bearing):
 * spdx-satisfies(finding, allowlist) is the wrong primitive for deny — it
 * treats the allowlist as "available licenses" and calls an OR finding
 * satisfied when any branch is available, so
 * satisfies("MIT OR BUSL-1.1", ["BUSL-1.1"]) is true. That would wrongly deny
 * a dependency that can elect MIT instead. The correct rule is the dual of
 * isCopyleft's recursion: a finding is denied only when it has no branch left
 * to elect out of the deny set.
 *   - leaf       → denied iff the leaf satisfies the deny allowlist;
 *   - OR (l, r)  → denied iff both sides are denied (one electable branch
 *                  defeats the denial);
 *   - AND (l, r) → denied iff either side is denied (an AND conjunct cannot
 *                  be elected away — every obligation applies).
 ...
 * This keeps deny exactly consistent with the compatible election path while
 * preventing over-denial of a finding that has an acceptable branch.
 *
 * One election runs across every license rule, not one per rule
 * (load-bearing): it must run against the union of every match="license"
 * deny allowlist, not each rule's allowlist in isolation. The shipped
 * defaults provide BUSL-1.1, SSPL-1.0, and Elastic-2.0 as separate
 * match="license" rules, so an isolated per-rule election sees only one
 * branch of "BUSL-1.1 OR SSPL-1.0" and never denies it — each rule finds the
 * other branch electable on its own. The correct decision builds the
 * combined allowlist once and asks nodeDenied against it: "BUSL-1.1 OR
 * SSPL-1.0" then has no electable branch and is denied, while "MIT OR
 * BUSL-1.1" stays electable (MIT is in neither allowlist). When the union
 * denies, the verdict is attributed to the first license rule that
 * contributes a denied leaf, for the rule-id/reason. Name-mode rules stay
 * per-rule — an exact name compare has nothing to elect.
 *
 ...
 * also lists wins attribution to their explicit rule; the deny election
 ...
 * first per effectiveDenyRules order. A null/unparseable expression can never
 * be license-denied. Returns undefined when no license rule, or no
 * electable-out branch, applies.
 */
...
/**
 * First deny rule that matches, or undefined, over the effective deny set
 * (policy denies, then shipped source-available defaults). Name-mode matches
 ...
 * earliest-listed matching rule wins — a name rule before a contributing
 * license rule is cited first, matching the documented "first deny rule"
 * precedence.
 */
