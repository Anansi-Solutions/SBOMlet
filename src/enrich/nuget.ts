/**
 * NuGet registration-API license resolution: the fixed-host URL builder, the catalogEntry host pin,
 * and the pure metadata-class ladder.
 *
 * License lookup is a TWO-STEP fetch against static, versioned CDN blobs: the registration leaf
 * `/v3/registration5-gz-semver2/{id}/{version}.json` carries a `catalogEntry` URL, and THAT
 * document carries the license fields. Casing is load-bearing: the leaf URL 404s unless BOTH the id
...
/**
 * The registration leaf URL for a purl-derived id + version. BOTH path parts are decoded from their
 * purl encoding, LOWERCASED (the registry 404s a mixed-case path - verified differential), and
...
/**
 * The catalogEntry URL from a registration leaf, HOST-PINNED: only a string that starts with the
 * `https://api.nuget.org/` prefix (trailing slash INCLUDED, so `api.nuget.org.evil.example` can
 * never pass) is returned. A missing field, a non-string, or ANY other host yields undefined - the
 * caller treats that as malformed (a clean negative, no request ever made to a foreign host),
 * closing the response-derived SSRF hole.
 */
...
/**
 * Resolve a raw license from a NuGet catalogEntry document - the pure four-class metadata ladder
...
/**
 * decodeURIComponent for the licenses.nuget.org URL path: yields undefined on a malformed escape or
 * ANY control character (never throws), and trims surrounding whitespace so a blank remainder falls
 * through to the honest unknown. The field is package-author-controlled and the decoded string
 * becomes a raw license claim - an SPDX expression is plain printable text, so a C0/C1 control
 * (newline forgery, ANSI erase) is a log/document injection vector, never a license.
 */
