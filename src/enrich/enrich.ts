 ...
 *
 * Finds every package whose current claims resolve to unknown (mirroring findingFromClaims'
 * emptiness/any-unresolvable test) and, in GENERATE mode, fetches each from its ecosystem registry
 ...
 * claim), so `clarify > registry > generator` precedence holds for free via annotateFindings
 * downstream. The input model is never mutated: new entries are produced via object spread, like
 * annotateFindings.
 */
...
/**
 * Parse `pkg:<type>/<name>@<version>`. The name may contain a "/" (scoped npm) and stays
 * verbatim/URL-encoded; the version is everything after the LAST "@" (a scoped name's leading "@"
...
export function npmPackumentUrl(encodedName: string): string {
  // "%40babel/core" → decode → "@babel/core" → encode the "/" only → the registry path
  // "@babel%2Fcore"; an unscoped "lodash" is unchanged.
...
/**
 * The terraform/github version-ref path. For each miss, derive the source repo from the registry
 * naming convention and try the ordered candidate refs (v<version> → <version> → default branch).
 * The FIRST ref that returns a clean 200 with a resolvable license wins; a 404 advances to the next
 * candidate (a missing-tag signal, NOT a terminal failure). A GithubTransientError (403 rate-limit
 * / 5xx / network / timeout) propagates LOUDLY (hard-fail, no entry). A non-conventional/non-github
 * source, or a clean answer with no license across all refs, is a DEFINITIVE no-license → a
...
async function fetchTerraformMisses(
  ...
  await mapLimit(misses, FETCH_CONCURRENCY, async (miss): Promise<void> => {
    ...
    // A clean 404 across ALL candidate refs (or NOASSERTION everywhere) → a DEFINITIVE no-license
    // answer → a governed negative entry.
    recordNegative(miss, cache, "github");
  });
}
