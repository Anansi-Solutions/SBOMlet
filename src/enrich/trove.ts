 ...
 * bare "ISC license"/"ISC License" label (correct() returns null for all three; the last is the
 * suffix false-negative that dropped pexpect to unknown). A handful of precise classifiers
 * (MIT/ISC/MPL) are included for exact resolution where the `license` field is empty.
 *
 ...
 * so a resolver tags them LOW confidence for optional `[[clarify]]` pinning, never a silent HIGH
 * verdict.
 *
 * Every SPDX value here is validated against spdx-license-ids in the tests, so a typo cannot
 * silently create a resolution gap (mirrors copyleft.ts's contract).
 */
...
> = [
  ...
  // a registry-supplied label resolves to the precise ISC id.
  ["ISC license", "ISC"],
  ["ISC License", "ISC"],
