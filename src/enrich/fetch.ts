 ...
 * built from a fixed registry base + URL-encoded purl-derived name/version, so an
 * attacker-controlled host is impossible by
 ...
 *
 * No custom `Accept` header is sent: the npm packument endpoint returns 406 Not Acceptable for
 * `Accept: application/vnd.npm.install-v1+json`, so the default JSON Accept is used.
 */
...
/**
 * Run `fn` over `items` with at most `limit` concurrent workers, returning results in input order.
 * A `limit` larger than `items.length` spawns only as many workers as there are items (no idle
 * over-workers).
 */
 ...
 * revision-E transient-vs-definitive distinction:
 *
