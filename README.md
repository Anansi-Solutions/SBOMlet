...
It covers JS/TS, Python, .NET, Maven, Terraform/OpenTofu, and the packages in your
Docker images — their full contents, the OS layer and the application packages on
top. It has no dependency on the project it audits — you add
...
1. **Point.** SBOMlet walks the repo for the places dependencies are declared:
   ...
   `uv.lock`, `packages.lock.json`, `maven.sbom.json`, `.terraform.lock.hcl`.
...
| Maven | `maven.sbom.json` — opt-in, one per module; see the [adoption prerequisite](docs/getting-started.md#if-your-repository-is-maven) |
