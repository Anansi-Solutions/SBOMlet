...
describe("firstPartyNames — workspace/portal member set from lockfile text", () => {
  test("verified real-world portal and workspace entry headers yield their names", () => {
    ...
    expect(firstPartyNames(lockfile)).toEqual(new Set(["iframe-rpc-react", "myapp"]));
  });

  test("scoped names capture the full @scope/name (lazy match does not stop at the leading @)", () => {
    const lockfile = '"@scratch/foo@workspace:apps/x":\n  version: 0.0.0-use.local\n';
...
describe("nugetThirdPartyEntryCount — entries with type !== 'Project' across all sections", () => {
  ...
  test("empty dependency sections are a positively-determined ZERO (warn+skip branch)", () => {
    expect(nugetThirdPartyEntryCount('{"version":2,"dependencies":{"net9.0":{}}}')).toBe(0);
  });
  ...
  test("no dependencies map → undefined (a lock that proves nothing is unknown, not zero)", () => {
    ...
    expect(nugetThirdPartyEntryCount('{"version":2,"dependencies":42}')).toBeUndefined();
  });
  ...
  test("an UNSUPPORTED lock version → undefined, never a positively-determined zero", () => {
    ...
    expect(nugetThirdPartyEntryCount('{"version":3,"dependencies":{"net9.0":{}}}')).toBeUndefined();
...
describe("pnpmThirdPartyEntryCount — packages: section key lines", () => {
  ...
  test("CRLF lockfiles parse identically", () => {
    expect(pnpmThirdPartyEntryCount(PNPM_V9_LOCKFILE.replace(/\n/g, "\r\n"))).toBe(3);
  });
...
describe("yarnWorkspaceMembers — @workspace: resolution body lines", () => {
  ...
  test.each([
    ...
    ["dots, dashes, underscores, digits", "my.pkg-name_2", "libs/my.pkg-name_2"],
    ...
    ["node_modules-adjacent name", "node-modules-helper", "packages/node-modules-helper"],
    ...
    ["uppercase name differing in case from a lowercase path", "Frontend", "Frontend"],
  ])(
    ...
    (_label, name, relPath) => {
      ...
      expect(yarnWorkspaceMembers(lockfile)).toEqual([{ name, relPath, hasDependencies: true }]);
    },
  );
});
...
describe("mavenThirdPartyEntryCount — components count excluding the doc's own root purl", () => {
  ...
  test("valid JSON that is not CycloneDX -> undefined", () => {
    expect(
      mavenThirdPartyEntryCount(JSON.stringify({ notes: "unrelated", components: [] })),
    ).toBeUndefined();
  });
