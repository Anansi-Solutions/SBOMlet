...
describe("comment density budget", () => {
  ...
  test("comment lines stay under the line budget", () => {
    expect(total.commentLines).toBeLessThanOrEqual(MAX_COMMENT_LINE_RATIO * total.totalLines);
  });

  test("comment words stay under the word budget", () => {
    expect(total.commentWords).toBeLessThanOrEqual(MAX_COMMENT_WORD_RATIO * total.totalWords);
  });
});
