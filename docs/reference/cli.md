...
- `suggest-clarifications` — read-only, offline; print ready-to-paste
  `[[clarify]]` stubs for NuGet packages whose licence is an honest
  unknown.
...
- A package whose registry metadata carries only a bare, pre-2019
  ...
  stays an honest unknown by design. Run
  [`suggest-clarifications`](#suggest-clarifications) to get a ready-to-paste
  `[[clarify]]` starting point for these packages. A cache entry committed
  before this arm existed still reads `resolvable: false` for these
  packages and is never re-fetched; delete the affected entries (or the
  cache file) and re-run `generate` to let the arm attempt them —
  `suggest-clarifications` counts these older entries and prints the same
  reminder.
...
Read-only and fully offline: it never touches the network and never writes
anything. It reads the committed
[enrichment cache](../glossary.md#enrichment-and-the-enrichment-cache) and
scans the repository exactly as `check` does, then prints ready-to-paste,
fully commented `[[clarify]]` stubs for NuGet packages whose only registry
evidence is a bare `licenseUrl` it could not follow (see
[.NET targets](#net-targets) above) and that have no `[[clarify]]` entry yet.

Every stub states only facts it observed: the package name, its version, and
the licence URL verbatim, exactly as the registry reported it. `expression`
and `reason` are always left as a fill-in placeholder for you — the tool
never decides a licence. `evidence_url` is pre-filled only when the observed
URL is already an immutable GitHub blob permalink (a `.../blob/<40-hex-sha>/...`
link); every other shape, including a `raw.githubusercontent.com` link
pinned to a commit SHA, stays a placeholder, because the schema's own
`evidence_url` field accepts only the `github.com/blob/<sha>` form. It never
follows a redirect and never invents a citation.

Every line the command prints is a TOML comment, so pasting the whole output
into your policy file changes nothing until you uncomment a block and fill
in `expression` and `reason` — the existing policy schema rejects an
uncommented entry that is still missing either one. Packages sharing one
observed URL are grouped into a single stub (the
[multi-package `[[clarify]]` form](policy.md#clarify)); a package whose name
starts with `runtime.` is split into its own stub, labelled for individual
review, because it wraps native (often non-.NET) code and should not inherit
a managed-library verdict.
...
# ]
# # observed licenseUrl (verbatim, unfollowed): https://go.microsoft.com/fwlink/?LinkId=329770
...
# # reason: cite exactly what you read and where
# reason = "FILL-ME-IN"
# # evidence_url: an immutable GitHub blob permalink — open the file on github.com and press "y" to get one
# evidence_url = "FILL-ME-IN"
```

Packages already covered by a `[[clarify]]` entry are excluded, so re-running
the command after you resolve some of them prints a shorter list each time,
converging to nothing once every honest unknown has a decision.

| Flag | Meaning | Default |
| --- | --- | --- |
| `--repo-root <path>` | Discovery mode: scan every target under this root. | current directory |
| `--target <path>` | Single-target mode: scan one target. | — |
| `--exclude <glob>` | Drop targets whose identity matches the glob. Repeatable. | none |
| `--policy <path>` | TOML policy file, read for its existing `[[clarify]]` entries. | none |
| `--base-dir <path>` | Anchor every relative path flag to this directory. | working directory |
| `--enrichment-cache <path>` | The committed enrichment cache to read. | tool default |
| `--verbose` | Print per-stage progress to stderr. | off |
...
The stub set prints to standard output — the one channel meant for pasting
into a file — and every message, including the count of committed cache
entries written before this command existed, prints to stderr. There is
nothing to gate: it always exits `0`, even when there is nothing to suggest,
unless it hits a config error (exit `3`).

```
task sbomlet:suggest:clarifications >> .sbomlet.policy.toml
```

(See [Taskfile entry points](#taskfile-entry-points) below — `--repo-root` and
`--policy` are the `REPO_ROOT` and `POLICY` variables.)
...
| `task sbomlet:suggest:clarifications` | `suggest-clarifications`; prints ready-to-paste `[[clarify]]` stubs. | While onboarding a .NET repository's honest-unknown packages. Read-only, offline, nothing written. |
...
| --- | --- | --- | --- |
| `REPO_ROOT` | `generate`, `check`, `suggest:clarifications`, `verify:cache`, `docker:list` | Repository root to scan; also the walk root for Dockerfile discovery. | the root Taskfile's directory |
...
| `POLICY` | `generate`, `check`, `suggest:clarifications`, `docker:list` | Policy file; the `--policy` flag is added only when set. | unset (no policy) |
