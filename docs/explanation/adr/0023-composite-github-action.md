...
tool + a Taskfile include + mise" and kept distribution as git + a mise-pinned bun
rather than a compiled binary. That is a clean local and cross-CI story, but on
GitHub Actions — where many consumers live — it still means wiring mise, Task, and
the include by hand. That is exactly the "still not one line" cost ADR-0019
accepted.
...
3. **A Docker-container action** — would pin a base image and a second toolchain,
   the redundant-runtime trap ADR-0019 rejected for the binary.
...
and `task <mode>` against the consumer's checkout. A consumer adds
`uses: Anansi-Solutions/SBOMlet@<ref>` with a `policy` and optional `mode` input.

Two properties matter. It is the EXISTING pipeline, not a fork: the action drives
the same Taskfile a local or non-GitHub CI run does, so there is no second
behaviour to keep in sync — only a thin shell around it. And inputs flow through the
environment, never template interpolation, so a crafted input can never reach shell
parsing — the same property the Taskfile holds. A dogfood workflow runs the action
against this repository on every push, so the action is gated by CI rather than only
...
- **Good:** no code-path divergence; the Taskfile and the action share the same
  `task` entry points.
...
- [ADR-0019](0019-adoption-git-clone-taskfile.md) (adoption by git clone + Taskfile,
  and the git + bun distribution) — this adds a one-line front door over the same
  pipeline.
